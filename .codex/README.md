...
`Codex` 팀은 이 디렉터리를 시작점으로 사용한다. 목적은 제품 코딩이 아니라 저장소 운영층과 `worktree harness`를 표준화하는 것이다.

## 현재 상태

- 현재 `phase`는 `.codex/config/harness.json` 기준 `preclean`
- 이 `phase`에서는 `docs`, `chore`, `spike` 브랜치만 허용
...
- `scripts/Get-TeamWorktreeStatus.ps1`: 공용 제어 `checkout`을 포함한 `worktree` 현황 조회
...
4. 공유 기준선이 커밋된 뒤에만 `.\.codex\scripts\New-TeamWorktree.ps1`로 팀 `worktree`를 만든다.
...
- `Claude 팀`은 기존 `.claude/`와 `.moai/`를 계속 사용한다.
- `Codex 팀`은 `AGENTS.md`와 `.codex/`를 기준으로 움직인다.
